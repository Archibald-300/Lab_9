...
class Gun():
    ...
    def move(self, add_x, add_y, time = 1):
        ...
        if (self.coord[0] > 30 or add_x > 0) and (self.coord[1] < SCREEN_SIZE[0] - 30 or add_x < 0):
    ...
    def strike(self, not_manna_ball):
        ...
        if not_manna_ball == True: 
            ...
            ball = Ball(list(self.coord), [int(speed * 0.7 * np.cos(angle)), int(speed * 0.7 * np.sin(angle))], 0.9, 0.9, ball_radius = 60, color = LIGHT  )
...
class New_target(Ball):
    ...
    def draw_smile(self, screen):
        ...
        for i in range(1, 3, 1):
            circle(screen, BLACK, (self.coord[0] + (self.ball_radius * (-1) ** i)// 2, self.coord[1] - self.ball_radius // 3), 
                self.ball_radius // (2 + i))
            circle(screen, RED, (self.coord[0] + (self.ball_radius * (-1) ** i )// 2, self.coord[1] - self.ball_radius // 3), 
                self.ball_radius // (2 + i) - 1, self.ball_radius // (8 * i))
        #drawing of mouth
        rect(screen, BLACK, ( self.coord[0] - self.ball_radius// 2, self.coord[1] + self.ball_radius// 3, self.ball_radius, 
            self.ball_radius // 7))
        #drawing of eyebrows
        for i in range(2):
            line(screen, BLACK, (self.coord[0], self.coord[1] - self.ball_radius // 2), (self.coord[0] + ((-1) ** i ) * self.ball_radius,
...
class Bomb():
    ...
    def grow_shadow(self):
        self.time += 0.5
        rect(screen, GRAY, (self.coord[0] - 4 * int(self.time), self.coord[1] - 5 * int(self.time), 8 * int(self.time), 10 * int(self.time)))
...
class editor():
    ...
    def new_mission(self):
        ...
        if self.score_table.score() < 0:
            ...
            for j in range(self.number_new_targets):
                self.new_targets.append(New_target(YELLOW, ball_radius = randint(max(1, 30 - 2 * self.score_table.score()), 30 - self.score_table.score()))) 
    ...
    def check_bomb(self):
        '''Initiate bomb, looks if gun in bomb's area, finnishes the game '''
        done = 0
        self.time += 1
        if self.time >= 40:
...
while done == 0:
    ...
    score_surface1 = font.render("Press 'f' to start machine's turbo motion and 'g' to stop it", True, army_color_1)
    screen.blit(score_surface1, [250, 25])
    done = edit_events.process(pygame.event.get(), screen)
    pygame.display.flip()

if done == 2:
    #shows "Game over" table after tank's smahed by bomb
    screen.fill(GREEN)
